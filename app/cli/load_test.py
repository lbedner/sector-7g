        ...
        if wait:
            if result is None:
                ...
                rprint(
                    ...
                    f"sector-7g load-test results {task_id}"
                )
            else:
            ...
            rprint(
                f"   [bold]sector-7g load-test results "
                f"{task_id}[/bold]"
            )
...
def show_test_type_info(
    ...
    if test_type:
        ...
        rprint("   sector-7g load-test info cpu_intensive")
        rprint("   sector-7g load-test info io_simulation")
        rprint("   sector-7g load-test info memory_operations")
...
def _display_error_result(result: dict[str, Any], status: str) -> None:
    ...
    console.print(
        f"   2. Verify system health: sector-7g health check"
    )
    console.print(
        "   3. Try a smaller load test first: "
        f"sector-7g load-test cpu --tasks 10"
    )
